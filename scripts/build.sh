...
BUILD=build/FrostyEQ_artefacts/RelWithDebInfo
...
stale=0
for pair in "VST3/FrostyEQ.vst3:VST3/FrostyEQ.vst3" "AU/FrostyEQ.component:Components/FrostyEQ.component"; do
    built="$BUILD/${pair%%:*}/Contents/MacOS/FrostyEQ"
    installed="$INSTALL/${pair##*:}/Contents/MacOS/FrostyEQ"
...
if (( stale )); then
    ...
    echo "  cmake --build build --target FrostyEQ_VST3 FrostyEQ_AU"
    exit 1
fi

[[ ${1:-} == --test ]] && ctest --test-dir build --output-on-failure
[[ ${1:-} == --run  ]] && open "$BUILD/Standalone/FrostyEQ.app"
